...
// 수의사 → 기존 폴더 경로 (슬래시 / 표기, 맨 앞 / 필수)
  ...
  const VET_PATHS = {
  "김지현": "/원내공유용/01. 수의사/000. 입원환자처치표/김지현",
  "김형준": "/원내공유용/01. 수의사/000. 입원환자처치표/김형준",
  "문정현": "/원내공유용/01. 수의사/000. 입원환자처치표/문정현",
  "민금주": "/원내공유용/01. 수의사/000. 입원환자처치표/민금주",
  "송애라": "/원내공유용/01. 수의사/000. 입원환자처치표/송애라",
  "유수민": "/원내공유용/01. 수의사/000. 입원환자처치표/유수민",
  "이소민": "/원내공유용/01. 수의사/000. 입원환자처치표/이소민",
  "정소연": "/원내공유용/01. 수의사/000. 입원환자처치표/정소연",
  "조문주": "/원내공유용/01. 수의사/000. 입원환자처치표/조문주",
  "한아름": "/원내공유용/01. 수의사/000. 입원환자처치표/한아름",
  "간호팀": "/원내공유용/02. 간호사",
  "리셉팀": "/원내공유용/03. 매니져",
};
...
async function uploadToDropbox(token, path, buffer){
  const apiArg = asciiSafeJson({ path, mode:"add", autorename:true, mute:true });
  const up = await fetch("https://content.dropboxapi.com/2/files/upload", {
    method:"POST",
    headers:{
      "Authorization": `Bearer ${token}`,
      "Dropbox-API-Arg": apiArg,           // JSON 그대로(퍼센트 인코딩 금지)
      "Content-Type":"application/octet-stream"
    },
    body: buffer
  });
  if(!up.ok){ const t = await up.text(); throw new Error("Dropbox upload 실패: " + t); }
  return up.json();
}
...
export default async function handler(req, res){
  try{
    ...
    if(req.method === "GET"){ res.status(200).json({ ok:false, hint:"POST /api/upload 로 파일 업로드", method:"GET" }); return; }
    if(req.method !== "POST"){ res.status(405).json({ok:false, error:"Method not allowed"}); return; }

    // 서버리스 환경 안전 설정
    const form = formidable({ multiples:false, maxFileSize: MAX_FILE_BYTES + 1024*1024, uploadDir:"/tmp", keepExtensions:true });
    ...
    if(!f || !f.filepath) return res.status(400).json({ok:false, error:"업로드된 파일 없음"});
